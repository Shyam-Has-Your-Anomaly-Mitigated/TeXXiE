...
\subsection{Dailies}
\item \texttt{Touch Typing\phantom{ng}:} \textit{67} WPM @ \textit{98}\% accuracy, over \href{https://www.typing.com/student/test/5}{5 minutes}.%(My computer is lagging; I usually get 10 more WPMs, but I had to stop, and wait for my computer to catch up to see if there were any bugs...)
\item \texttt{$\text{\texttt{2}}^\text{\texttt{3}}$ Speedcubing:} \textit{00}:\textit{20}.\textit{94} via \href{http://www.cubetimer.com/}{this link}.
\item \texttt{$\text{\texttt{3}}^\text{\texttt{3}}$ Speedcubing:} \textit{02}:\textit{06}.\textit{72} via \href{http://www.cubetimer.com/}{this link}.
\item \texttt{$\text{\texttt{3}}^\text{\texttt{3}}$ Blindfolded:} \textit{07}:\textit{09}.\textit{90} via \href{http://www.cubetimer.com/}{this link}.
