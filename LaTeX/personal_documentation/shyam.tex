...
\title{\begin{center}\includegraphics[keepaspectratio,width=\textwidth]{shyam.png}\end{center}}
...
\vfill
\abstract{
    This is my personal documentation, and it describes my curriculum vitæ (\sh{résumé}); which $\mathbb{R}$eally means I'm attempting to \textit{con}vince you to give me your money.
...
Hello World,
\smallskip\\\par{}My name is SHYAM; an infinitely recursive acronym, which stands for ``\sh{SHYAM Has Your Anomaly Mitigated}'', and speaks for itself asynchronously, but it's pronounced ``\sh{Sharm}''.% self-explanatory
...
\begin{description}
    ...
    \begin{description}
        ...
        \item \texttt{*\phantom{*}\href{https://www.open.edu.au/subjects/rmit-university-building-it-systems-rmi-cpt111}{CPT111}:} \uniHD{Building IT Systems} (\sh{Lisp, Clojure, ClojureScript, Discord.js})
        ...
        \item \texttt{\phantom{**}\href{https://www.open.edu.au/subjects/rmit-university-software-engineering-fundamentals-rmi-cpt230}{CPT230}:} \uniD{Software Engineering Fundamentals} (\sh{\href{https://en.wikipedia.org/wiki/Software_engineer}{UML}})
        ...
        \item \texttt{*\phantom{*}\href{https://www.open.edu.au/subjects/rmit-university-professional-computing-practice-rmi-cpt310}{CPT310}:} \uniD{Professional Computing Practice} (\sh{\href{https://en.wikipedia.org/wiki/Professional}{Cybercrime, Full Disclosure, Whistleblowing}})
    ...
    \begin{description}
        \item \texttt{\phantom{**}\href{https://www.open.edu.au/subjects/rmit-university-programming-in-c-rmi-cpt220}{CPT220}:} \uniD{Programming in C} (\sh{\href{https://en.wikipedia.org/wiki/System_Programmer}{C}})
        \item \texttt{\phantom{**}\href{https://www.open.edu.au/subjects/rmit-university-software-architecture-design-and-implementation-rmi-cpt222}{CPT222}:} \uniP{Software Architecture: Design and Implementation} (\sh{\href{https://en.wikipedia.org/wiki/Software_architect}{Java, UML, MVC, GoF}})
        ...
        \item \texttt{\phantom{**}\href{https://www.open.edu.au/subjects/rmit-university-object-oriented-programming-in-c-rmi-cpt323}{CPT323}:} \uniHD{Object-Oriented Programming in C++} (\sh{\href{https://en.wikipedia.org/wiki/Video_game_programmer}{C++}})
        \item \texttt{\phantom{**}\href{https://www.open.edu.au/subjects/rmit-university-discrete-maths-rmi-mat17}{MAT17\phantom{*}}:} \uniHD{Discrete Maths} (\sh{\href{https://en.wikipedia.org/wiki/Mathematician}{\texttt{/.+/}, and Formal Methods including Hoare, Logic, and Induction}})
...
I will study a B.IT by 2019-11-27, incur a substantial robodebt, and fall victim to the hype cycle.% the bane of my existence; you just can't beat the learning curve!
...
\noindent{}I will study for my \href{https://www.handbook.unsw.edu.au/postgraduate/specialisations/2021/COMPAS}{M.IT @ UNSW}; I assembled a dependency graph for my \textit{Academic Timeline}, and now I'm waiting for Coronavirus to cease, and desist.% $\because$ they allow up to two specialisations; ``\sh{Artificial Intelligence}'' is locked-in as my primary specialisation.
...
\begin{itemize}
	\item Core Subjects
		\begin{description}
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9021}{COMP9021}:} Principles of Programming (\sh{Python})
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9024}{COMP9024}:} Data Structures and Algorithms (\sh{C})
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9311}{COMP9311}:} Database Systems (\sh{SQL, PL/pgSQL, SQLite})
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9331}{COMP9331}:} Computer Networks and Applications (\sh{TCP/IP})
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9814}{COMP9814}:} Extended Artificial Intelligence (\sh{Prolog, Lisp, Computer Vision})
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9900}{COMP9900}:} $\text{Information Technology Project (\sh{Formal Methods, Artificial Intelligence, Prolog, Lisp, Hex})}$
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/GSOE9820}{GSOE9820}:} Engineering Project Management (\sh{Scrum})
		\end{description}
	\item Specialisation: Artificial Intelligence (\sh{Declarative Metaprogramming = Safety + Productivity})% (better v faster v cheaper) = (safety v performance v productivity)
		\begin{description}
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP4418}{COMP4418}:} Knowledge Representation and Reasoning (\sh{Prolog, Potassco})
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9417}{COMP9417}:} Machine Learning and Data Mining (\sh{R})
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9844}{COMP9844}:} Extended Neural Networks and Deep Learning (\sh{PyTorch, Image Processing}) % Python, Torch, NumPy, Sklearn
		\end{description}
	\item Electives: Formal Methods $\text{(\sh{Superpositional Concurrency = Safety + Performance})}$% (better v faster v cheaper) = (safety v performance v productivity); formal methods = long-term productivity #roi (as long as it doesn't need to be maintained forever after)
		\begin{description}
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP4161}{COMP4161}:} Advanced Topics in Software Verification (\sh{Isabelle/HOL, C, AutoCorres})
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP6721}{COMP6721}:} (In-)Formal Methods: The Lost Art (\sh{Python, C, Dafny})
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9153}{COMP9153}:} $\text{Algorithmic Verification (\sh{Model Checking including SPIN, Skink, and, UPPAAL})}$ % CBMC, nuXmv, NuSMV, GOANNA, CTL=Computation_Tree_Logic, LTL=Linear-time_Temporal_Logic, symbolic model checking sounds like the best
		\end{description}
	\item Electives: Artificial Intelligence (\sh{Formal Intelligence = Safety + Performance + Productivity})% (better v faster v cheaper) = (safety v performance v productivity)
		\begin{description}
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9434}{COMP9434}:} Robotic Software Architecture (\sh{ROS, Computer Vision})
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9491}{COMP9491}:} Applied Artificial Intelligence (\sh{Computer Vision})
		\end{description}
	\item Dependency for \texttt{COMP6721}, \texttt{COMP9153}, and \texttt{COMP9417}
		\begin{description}
			\item \texttt{\href{https://www.handbook.unsw.edu.au/postgraduate/courses/2021/COMP9020}{COMP9020}:} Foundations of Computer Science (\sh{Discrete Maths})
		\end{description}
\end{itemize}
...
\subsection{Dailies}
\item \texttt{Touch Typing\phantom{ng}:} \textit{68} WPM @ \textit{99}\% accuracy, over \href{https://www.typing.com/student/test/5}{5 minutes}.%(My computer is lagging; I usually get 10 more WPMs, but I had to stop, and wait for my computer to catch up to see if there were any bugs...)
...
\begin{itemize}
	...
	\item FLOSSware
		\begin{itemize}
			\item Language Implementations (\sh{Lisp dialects \=c MIT license, Prolog dialects \=c EUPL licence,..})%(\sh{MIT-licensed Scheme, MIT-licensed ANSI Lisp, EUPL-licenced Edinburgh Prolog, EUPL-licenced ISO Prolog,..})
			\item Programming Tools (\sh{Emacs \=c \texttt{/Emacs|Gnu/} GPL license, Prolog,..})
			\item Operating Systems (\sh{Lisp, Prolog, \textit{\texttt{/.*/} Linux From Scratch}, BSD \=c BSD license, Unix,..})
		\end{itemize}
	\item Hacking% / Coding / Programming / Development / Engineering / CSIT
		\begin{itemize}
			\item $\text{Prolog (\sh{pseudo-coded ATP'ing Logic Theory Machine, OHSafe PNTemporal CYCommonsense,..})}$% pronounced "OPS", RAM = risk /assessment|management/
			\item (lisp\sh{, emacs, stumpwm, guix, acl2, GIMP, AUDACITY, SCSH,..})%LISP2DOT suxs; not just because it's awk
			\item Haskell (\sh{the ``-thing''s Lisp doesn't have…yet; whereafter, Hex Editing will replace this})%this will be replaced by
		\end{itemize}
	\item Mathematics (\sh{Formal Methods, \textit{The Calculi $\circ$ $\lambda$-Conversion},..})
%		\begin{itemize}
...
%	\end{itemize}
	\item Automation (\sh{Lisp, Prolog,..})
		\begin{itemize}
			...
			\item Macros (\sh{Lisp, Emacs, StumpWM, Prolog,..}) % Make, M4, Readline, Xdotool
		\end{itemize}
%	\begin{itemize}
%		\setlength\itemsep{0em}
...
%	\end{itemize}
	\item Technology
		\begin{itemize}
			\item Brain-Computer Interface (\sh{OpenBCI + yoga = technological singularity})% via Electrodiagnosis (\sh{OpenBCI})
			\item Vision Interface (\sh{GPGPU})
			\item $\text{Speech Interface (\sh{Elephant 2000 literate programming, \texttt{/signal|language/} processing, live coding,..})}$ % , WaveNet, mathemusic; literate programming 1984 + elephant 2000 1996; algol [48,50]
		\end{itemize}
%	\begin{itemize}
%		\setlength\itemsep{0em}
...
%	\end{itemize}
	\item Security
		\begin{itemize}
			...
			\item Firewalls (\sh{\texttt{/MAC|IP/} addresses, ports,..})
			\item Cracking (\sh{Qubits})
		\end{itemize}
	\item Games (\sh{LHS is proprietary, RHS is FLOSS $\because$ it's the Right $\circ$ Way})
		\vspace{-0.25cm}
		\begin{multicols}{2}
		\begin{itemize}
			\item Shenmue (\sh{Bell Ringing \& Puddle Leaping})%[\vspace{\fill}]
			\item Jade Empire $\text{(\sh{Way $\circ$ proprietary $\lor$ FLOSS})}$% Karmā %[\vspace{\fill}]
			\item Beyond Good \& Evil (\sh{Farscape Budong})%[\vspace{\fill}]
			\item Dead Or Alive Xtreme (\sh{Z\kern-0.45em\lower-0.05em\hbox{|}\kern0.175em})% 5,318,008 %[\vspace{\fill}]
			\item Ryzom $\text{(\sh{Pay If You Want})}$%[\vspace{\fill}]
			\item NetHack (\sh{To Be Played})%[\vspace{\fill}]
			\item Colossal Cave Adventure (\sh{\href{https://www.amc.com/shows/halt-and-catch-fire/exclusives/colossal-cave-adventure}{Now Playing}})%[\vspace{\fill}]
			\item The Ur-Quan Masters (\sh{FSM})%[\vspace{\fill}]
		\end{itemize}
		\end{multicols}
\end{itemize}
\vspace{-.75cm}
