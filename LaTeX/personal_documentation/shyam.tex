...
\begin{description}
    ...
    \begin{description}
        ...
        \item \texttt{\phantom{**}\href{https://www.open.edu.au/courses/it/rmit-university-software-architecture-design-and-implementation--cpt222-2017}{CPT222}:} \uniP{Software Architecture: Design and Implementation} $\text{(\sh{Java, UML, MVC, GoF})}$
...
\subsection{Dailies}
\item \texttt{Touch Typing\phantom{ng}:} \textit{66} WPM @ \textit{98}\% accuracy, over \href{https://www.typing.com/student/test/5}{5 minutes}.%(My computer is lagging; I usually get 10 more WPMs, but I had to stop, and wait for my computer to catch up to see if there were any bugs...)
\item \texttt{$\text{\texttt{2}}^\text{\texttt{3}}$ Speedcubing:} \textit{00}:\textit{17}.\textit{85} via \href{http://www.cubetimer.com/}{this link}.
\item \texttt{$\text{\texttt{3}}^\text{\texttt{3}}$ Speedcubing:} \textit{01}:\textit{30}.\textit{99} via \href{http://www.cubetimer.com/}{this link}.
\item \texttt{$\text{\texttt{3}}^\text{\texttt{3}}$ Blindfolded:} \textit{12}:\textit{54}.\textit{53} via \href{http://www.cubetimer.com/}{this link}.
